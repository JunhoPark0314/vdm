...
class VDM(nn.Module):
  ...
  def __call__(self, images, conditioning, T_eval, deterministic: bool = True, use_t_eval: bool = False):
    ...
    z_latent_fft = jnp.fft.rfft2(z_latent, axes=[1,2])
    z_latent_diff = (jnp.square(z_latent_fft.real) + jnp.square(z_latent_fft.imag)) / (self.z_std * 0.5)

    z_1_eps_fft = jnp.fft.rfft2(z_1_eps, axes=[1,2])
    z_1_eps_diff = (jnp.square(z_1_eps_fft.real) + jnp.square(z_1_eps_fft.imag)) / (self.z_std * 0.5)

    # plog_p = -(0.5 * (1 + jnp.log(2 * jnp.pi)) + 0.5 * jnp.log(var_1 * self.z_std * 0.5)).sum() * 2

    plog_p = jnp.sum(-0.5 * z_1_eps_diff, axis=[1,2,3]) - 0.5 * jnp.log(jnp.pi * 2 * self.z_std * var_1 * 0.5).sum(axis=[1,2,3]) * 2
    log_q = jnp.sum(-0.5 * z_latent_diff, axis=[1,2,3]) - 0.5 * jnp.log(jnp.pi * 2 * self.z_std * 0.5).sum(axis=[1,2,3]) * 2
...
class ResnetBlock(nn.Module):
  ...
  def __call__(self, x, cond, deterministic: bool, enc=None):
    ...
    if cond is not None:
      ...
      h += FourierFeature(name='ff_block')(cond, h)
...
class ResNetAR(nn.Module):
  ...
  def setup(self):
    ...
    self.scale_coef = self.param('scale_coef', constant_init(jnp.log(1/(self.Resolution))), (mask_shape))
